...
const VideoPlayer = ({ src, thumbnail }: { src: string; thumbnail: string }) => {
  ...
  const videoRef: Ref<HTMLVideoElement> | undefined = useRef<any>(undefined)
    ...
    return (
      <>
        ...
        <button onClick={toggleMute}>
          {isMuted ? (
            <Icon icon={'ic:round-volume-mute'} />
          ) : (
            <Icon icon={'material-symbols-light:volume-up-rounded'} />
          )}
        </button>
        <input
          type="range"
        ...
        <button onClick={toggleFullScreen}>
          {isFullScreen ? <Icon icon={'uit:compress'} /> : <Icon icon={'uit:exapand'} />}
        </button>
      </>
    )
  }
