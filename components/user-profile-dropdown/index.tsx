...
export default function UserProfileDropdown({
  ...
  return (
    <div className={`relative flex h-fit w-fit min-w-16 flex-col ${className}`}>
      <button onClick={() => setIsOpen(!isOpen)} className="z-10 inline-flex items-center gap-1 rounded-full px-1.5 py-1.5 text-center text-sm font-medium focus:outline-none focus:ring-2">
        <Image className='rounded-full border border-textcolor/25' width={35} height={35} alt='blank-profile-picture' src={blackProfilePicture} />
        <Icon icon={'mynaui:chevron-down-solid'} className="h-4 w-4" />
      </button>
      {/*  */}

      <div
        className={`absolute right-0 -bottom-44 z-20 w-48 divide-y rounded shadow-sm ${isOpen ? 'block' : 'hidden'} `}
      >
