              ...
              >
                ...
                {menu.subMenus && (
                  <ol
                    ...
                    className={`absolute right-0 h-0 min-w-60 gap-4 overflow-hidden rounded-md bg-white text-sm shadow ${
                      subMenuClicked == menu.subPath && '!h-fit'
                    }`}
                  >
                    {menu.subMenus.map((subMenu, index) => (
                      <li key={index}>
                        <Link
                          className="flex w-full items-center justify-between border-4 border-transparent px-3 py-2 hover:border-l-primary hover:bg-secondary/50 hover:font-semibold hover:text-primary"
                          href={subMenu.external ? subMenu.path : menu.subPath + subMenu.path}
                          // href={subMenu.path}
                          target={subMenu.external ? '_blank' : '_self'}
                        >
